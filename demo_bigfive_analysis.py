...
def interactive_analysis():
    ...
    # 加载数据
    logger.info("[yellow]正在加载数据...[/yellow]")
    ...
    while True:
        # 创建菜单表格
        menu_table = Table(title="[bold cyan]功能菜单[/bold cyan]")
        ...
        menu_table.add_row("0", "[red]退出[/red]")
...
def show_data_overview(data):
    ...
    # 统计各问题的答案分布
    stats_table = Table(title="各问题答案分布统计")
...
def analyze_user_personality(analyzer, data):
    ...
    try:
        ...
        # 创建性格特征表格
        personality_table = Table(title="性格特征分析", show_header=True)
        personality_table.add_column("维度", style="cyan")
        personality_table.add_column("得分", style="yellow")
        personality_table.add_column("等级", style="green")
        ...
        answers_table = Table(title="原始答案", show_header=True)
        answers_table.add_column("问题", style="cyan")
        answers_table.add_column("答案", style="yellow")
...
def find_similar_users(analyzer, data):
    ...
    try:
        ...
        similar_table = Table(title="相似用户列表", show_header=True)
        similar_table.add_column("排名", style="cyan", width=6)
...
def generate_full_report(analyzer, data):
    ...
    try:
        ...
        console.print(Panel(report, title="[bold cyan]相似度分析报告[/bold cyan]", expand=False))
...
def visualize_personality_radar(analyzer, data):
    ...
    try:
        ...
        logger.info("[yellow]正在生成雷达图...[/yellow]")
        ...
        logger.warning("[yellow]请确保已安装matplotlib[/yellow]")
...
def visualize_similarity_heatmap(analyzer, data):
    ...
    """
    try:
        console.print(Panel("[bold cyan]选择可视化方式[/bold cyan]", expand=False))
        
        options_table = Table(show_header=False)
        options_table.add_column("选项", style="cyan")
        options_table.add_column("描述", style="white")
        options_table.add_row("1", "随机选择20个用户")
        options_table.add_row("2", "手动输入用户ID列表")
        options_table.add_row("3", "选择前N个用户")
        ...
        logger.info("[yellow]正在生成热力图...[/yellow]")
        ...
        logger.error(f"[red]可视化失败: {e}[/red]")
        logger.warning("[yellow]请确保已安装matplotlib和seaborn[/yellow]")
...
def batch_similarity_analysis(analyzer, data):
    ...
    try:
        ...
        logger.info(f"[yellow]正在分析前 {n_users} 个用户的相似度...[/yellow]")
        ...
        console.print(Panel(f"[bold green]最相似的用户对: 用户 {most_similar_pair[0]} 和 用户 {most_similar_pair[1]}\n相似度: {max_similarity:.4f}[/bold green]", expand=False))
        
        # 显示这两个用户的性格特征对比
        console.print(Panel("[bold cyan]性格特征对比[/bold cyan]", expand=False))
        ...
        comparison_table = Table(title="性格特征对比", show_header=True)
        comparison_table.add_column("维度", style="cyan")
        comparison_table.add_column(f"用户{most_similar_pair[0]}", style="green")
        comparison_table.add_column(f"用户{most_similar_pair[1]}", style="yellow")
        comparison_table.add_column("差异", style="red")
        
        for dim in profile1.keys():
            diff = abs(profile1[dim] - profile2[dim])
            comparison_table.add_row(dim, f"{profile1[dim]:.2f}", f"{profile2[dim]:.2f}", f"{diff:.2f}")
...
def main():
    ...
    主函数
    """
    console.print(Panel("[bold cyan]BigFive性格相似度分析程序[/bold cyan]", expand=False))
    
    # 检查数据文件是否存在
    data_path = "reasoner/dataset/dataset/bigfive.csv"
    if not os.path.exists(data_path):
        logger.error(f"[red]错误: 数据文件 {data_path} 不存在[/red]")
        logger.warning("[yellow]请确保数据文件路径正确[/yellow]")
        return
    
    try:
        interactive_analysis()
    except KeyboardInterrupt:
        logger.warning("[yellow]程序被用户中断[/yellow]")
    except Exception as e:
        logger.error(f"[red]程序运行出错: {e}[/red]")
        import traceback
        traceback.print_exc()
