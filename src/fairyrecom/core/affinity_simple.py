...
class VideoAffinityAnalyzer:
    ...
    def __init__(self, db_path: str = "data.db"):
        ...
        # TextBlob情感分析配置
        # TextBlob返回的情感极性范围是[-1, 1]，需要转换为[0, 1]
    ...
    def load_data(
        self, file_path: str = "reasoner/dataset/dataset/interaction.csv"
    ) -> pd.DataFrame:
        """
        加载用户-视频交互数据
    ...
    def calculate_sentiment_score(self, text: str) -> float:
        ...
        Returns:
            情感分数 (0-1)，其中0.5为中性，>0.5为正面，<0.5为负面
        """
        ...
        try:
            ...
            # 如果TextBlob返回中性但有明显的情感词汇，使用关键词分析
            if abs(textblob_score - 0.5) < 0.1 and (
                positive_count > 0 or negative_count > 0
            ):
                if positive_count > negative_count:
                    keyword_score = 0.7 + min(positive_count * 0.1, 0.3)  # 0.7-1.0
                elif negative_count > positive_count:
                    keyword_score = 0.3 - min(negative_count * 0.1, 0.3)  # 0.0-0.3
                else:
                    keyword_score = 0.5

                # 结合TextBlob和关键词分析结果
                final_score = (textblob_score + keyword_score) / 2
            else:
            ...
            # 如果分析失败，尝试简单的关键词分析
            logger.warning(f"情感分析失败: {e}，使用关键词分析")
            try:
                text_str = str(text).lower()
                if any(
                    word in text_str
                    for word in ["good", "great", "love", "like", "好", "棒", "喜欢"]
                ):
                    return 0.7
                elif any(
                    word in text_str
                    for word in ["bad", "hate", "terrible", "差", "讨厌", "垃圾"]
                ):
                    return 0.3
                else:
                    return 0.5
    ...
    def calculate_engagement_score(self, row) -> float:
        ...
        for tag_col in ["reason_tag", "video_tag", "interest_tag"]:
            if tag_col in row and pd.notna(row[tag_col]):
                try:
                    # 处理标签字符串
                    tag_str = str(row[tag_col])
                    if tag_str.startswith("[") and tag_str.endswith("]"):
                        # 计算逗号数量来估算标签数量
                        tag_count = tag_str.count(",") + 1 if "," in tag_str else 1
                        score += min(tag_count / 10, 0.2)  # 每个标签列最多贡献0.2分
                except:
                    pass
    ...
    def calculate_affinity_score(self, row) -> Dict[str, float]:
        ...
        # 综合喜爱度分数 (加权平均)
        weights = {"rating": 0.3, "like": 0.25, "sentiment": 0.25, "engagement": 0.2}
    ...
    def analyze_all_users(self) -> Dict[Tuple[int, int], Dict[str, float]]:
        ...
        logger.info(
            f"[green]分析完成，共处理 {len(affinity_results)} 个用户-视频对[/green]"
        )
    ...
    def save_to_database(self, batch_size: int = 1000):
        ...
        logger.info(
            f"[green]数据保存完成，共保存 {len(self.affinity_scores)} 条记录[/green]"
        )
...
#     主函数 - 演示视频喜爱度分析流程
#     """
#     logger.info("[bold blue]视频喜爱度分析系统 - 简化版[/bold blue]")
...
#             logger.info(
#                 f"[white]视频{video_id}: 喜爱度{affinity:.4f} (评分{rating:.4f}, 点赞{like:.4f}, 情感{sentiment:.4f}, 参与度{engagement:.4f})[/white]"
#             )
...
#             logger.info(
#                 f"[white]用户{user_id}: 喜爱度{affinity:.4f} (评分{rating:.4f}, 点赞{like:.4f}, 情感{sentiment:.4f}, 参与度{engagement:.4f})[/white]"
#             )
